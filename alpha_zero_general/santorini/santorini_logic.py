...
class Board:
    ...
    def get_moves_for_location(self, location: Tuple[int, int]) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
        ...
        move_locations = np.copy(
            self.pieces[0][
                max(x - 1, 0) : min(x + 2, self.n), max(y - 1, 0) : min(y + 2, self.n)
            ]
        )
        ...
        if valid_move_locations.shape[0] == 2:
            ...
            valid_move_locations = np.insert(
                valid_move_locations, 2 * (x != 0), False, axis=0
            )  # insert at index 0 if x ==0, else at index 2
        if valid_move_locations.shape[1] == 2:
            assert (y == 0) or (y == self.n - 1)
            valid_move_locations = np.insert(
                valid_move_locations, 2 * (y != 0), False, axis=1
            )  # insert at index 0 if y ==0, else at index 2

        assert valid_move_locations.shape == (3, 3)
        valid_moves = valid_move_locations.flatten()
        valid_moves = np.delete(
            valid_moves, 4
        )  # remove False value corresponding to not moving anywhere
        ...
        for move in moves:
            actions.extend(
                self.get_builds_for_location(move, [x - move[0], y - move[1]], location)
            )

        for move in all_moves:
            all_builds, all_builds_bool = self.get_all_builds_for_location(
                move, [x - move[0], y - move[1]], location
            )
    ...
    def get_builds_for_location(self, move: Tuple[int, int], offset: List[int], original_location: Tuple[int, int]) -> List[List[Tuple[int, int]]]:
        ...
        if valid_build_locations.shape[0] == 2:
            assert (x == 0) or (x == self.n - 1)
            valid_build_locations = np.insert(
                valid_build_locations, 2 * (x != 0), False, axis=0
            )
        if valid_build_locations.shape[1] == 2:
            assert (y == 0) or (y == self.n - 1)
            valid_build_locations = np.insert(
                valid_build_locations, 2 * (y != 0), False, axis=1
            )
        ...
        builds = list(
            map(lambda x: [tuple(original_location), tuple(move), tuple(x)], builds)
        )
    ...
    def get_all_builds_for_location(self, move: Tuple[int, int], offset: List[int], original_location: Tuple[int, int]) -> Tuple[List[List[Tuple[int, int]]], np.ndarray]:
        ...
        if valid_build_locations.shape[0] == 1:  # x is -1 or self.n
            if x < 0:
                valid_build_locations = np.insert(
                    valid_build_locations, 0, False, axis=0
                )
                valid_build_locations = np.insert(
                    valid_build_locations, 0, False, axis=0
                )
                ...
                valid_build_locations = np.insert(
                    valid_build_locations, 1, False, axis=0
                )
                valid_build_locations = np.insert(
                    valid_build_locations, 1, False, axis=0
                )
        ...
        if valid_build_locations.shape[1] == 1:  # y is -1 or self.n
            if y < 0:
                valid_build_locations = np.insert(
                    valid_build_locations, 0, False, axis=1
                )
                valid_build_locations = np.insert(
                    valid_build_locations, 0, False, axis=1
                )
                ...
                valid_build_locations = np.insert(
                    valid_build_locations, 1, False, axis=1
                )
                valid_build_locations = np.insert(
                    valid_build_locations, 1, False, axis=1
                )
