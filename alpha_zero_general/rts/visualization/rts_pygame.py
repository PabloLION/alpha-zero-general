...
def init_visuals(
    ...
    if verbose:
        ...
        pygame.init()
        canvas_scale = int(
            ctypes.windll.user32.GetSystemMetrics(1) * (2 / 3) / world_height
        )  # for drawing - it takes 2 thirds of screen height

        # square

        display_width, display_height = (
            world_width * canvas_scale,
            world_height * canvas_scale,
        )  # for example 800
...
def update_graphics(
    board: GenericBoardTensor, game_display, clock, fps: int = 1
) -> None:
    """
    ...
    for y in range(canvas_scale, (n + 2) * canvas_scale, canvas_scale):
        pygame.draw.line(
            game_display, (0, 0, 0), [y, canvas_scale], [y, (n + 1) * canvas_scale]
        )
        for x in range(canvas_scale, (n + 2) * canvas_scale, canvas_scale):
            pygame.draw.line(
                game_display, (0, 0, 0), [canvas_scale, x], [(n + 1) * canvas_scale, x]
            )
            if x < (n + 1) * canvas_scale and y < (n + 1) * canvas_scale:
                message_display(
                    game_display,
                    "" + str(x / canvas_scale - 1) + ", " + str(y / canvas_scale - 1),
                    ((x + canvas_scale / 4), (y + canvas_scale / 10)),
                    int(canvas_scale / 8),
                )
    ...
    message_display(
        game_display,
        "" + "Gold Player +1: " + str(gold_p1),
        (
            int((n / 8) * canvas_scale),
            (n + 1) * canvas_scale
            + int(
                int(canvas_scale / 12)
                + canvas_scale * (0 / 4)
                + int(canvas_scale * (1 / 8))
            ),
        ),
        int(canvas_scale / 6),
    )
    ...
    for y in range(n):
        for x in range(n):
            ...
            if a_player == 1 or a_player == -1:
                ...
                pygame.draw.circle(
                    game_display, actor_color, actor_location, actor_size
                )
                ...
                if a_type != 1:  # if not gold
                    message_display(
                        game_display,
                        "hp: " + str(actor_health),
                        (actor_x, actor_y + canvas_scale * (2 / 10)),
                        int(actor_size * 0.5),
                    )
                ...
                if a_type == 2:  # if npc
                    message_display(
                        game_display,
                        "carry: " + str(actor_carry),
                        (actor_x, actor_y + canvas_scale * (4 / 10)),
                        int(actor_size * 0.5),
                    )
