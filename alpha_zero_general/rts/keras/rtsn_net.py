...
class RTSNNet:
    def __init__(self, game, encoder):
        ...
        self.input_boards = Input(
            shape=(self.board_x, self.board_y, num_encoders)
        )  # s: batch_size x board_x x board_y x num_encoders

        x_image = Reshape((self.board_x, self.board_y, num_encoders))(
            self.input_boards
        )  # batch_size  x board_x x board_y x num_encoders
        ...
        h_conv3 = Activation("relu")(
            BatchNormalization(axis=3)(
                Conv2D(
                    CONFIG.nnet_args.num_channels, 3, padding="valid", use_bias=False
                )(h_conv2)
            )
        ...
        s_fc1 = Dropout(CONFIG.nnet_args.dropout)(
            Activation("relu")(
                BatchNormalization(axis=1)(Dense(256, use_bias=False)(h_conv4_flat))
            )
        )  # batch_size x 1024
        s_fc2 = Dropout(CONFIG.nnet_args.dropout)(
            Activation("relu")(
                BatchNormalization(axis=1)(Dense(128, use_bias=False)(s_fc1))
            )
        )  # batch_size x 1024
