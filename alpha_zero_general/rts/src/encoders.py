...
class Encoder:
    ...
    def encode(self, board: GenericBoardTensor) -> GenericBoardTensor:
        pass
...
class NumericEncoder(Encoder):
    def __init__(self) -> None:
        super().__init__()
        self.NUM_ENCODERS = (
            6  # player_name, act_type, health, carrying, money, remaining_time
        )
...
class OneHotEncoder(Encoder):
    ...
    def _build_indexes(self):
        ...
        self.MONEY_IDX_INC_OH = (
            8  # money-> 8 bits (255) [every unit has the same for player]
        )
    ...
    def encode(self, board: GenericBoardTensor) -> GenericBoardTensor:
        ...
        for y in range(n):
            for x in range(n):
                ...
                b[x, y][self.P_NAME_IDX_OH : self.P_NAME_IDX_MAX_OH] = self.itb(
                    player, self.P_NAME_IDX_INC_OH
                )
                ...
                b[x, y][self.CARRY_IDX_OH : self.CARRY_IDX_MAX_OH] = self.itb(
                    board[x, y, CARRY_IDX], self.CARRY_IDX_INC_OH
                )
                b[x, y][self.MONEY_IDX_OH : self.MONEY_IDX_MAX_OH] = self.itb(
                    board[x, y, MONEY_IDX], self.MONEY_IDX_INC_OH
                )
                b[x, y][self.REMAIN_IDX_OH : self.REMAIN_IDX_MAX_OH] = self.itb(
                    board[x, y, TIME_IDX], self.REMAIN_IDX_INC_OH
                )
        return b
